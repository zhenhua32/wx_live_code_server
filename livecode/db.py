...
async def init_gridfs_resource(app):
    ...
    def create_handler(pattern, collection, by='id', pattern_name=None):
        if by == 'id':
            handler = motor.aiohttp.AIOHTTPGridFS(client.ico, root_collection=collection,
    ...
    create_handler('/wx/file/live_code/{filename}', 'live_code_img', by='id', pattern_name='live_code_img')
    create_handler('/wx/file/user_img/{filename}', 'user_img', by='id', pattern_name='user_img')
